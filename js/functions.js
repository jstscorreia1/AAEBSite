...
/* --------------------------------------------------------	
   ...
   --------------------------------------------------------	*/	
	...
	$('#send').click(function(){ // when the button is clicked the code executes
		...
		if(message == "" || message == " " || message == "Mensagem") {
...
/* Vania - copiei a função porque esse email tem um campo a mais */
	$('#sendA').click(function(){ // when the button is clicked the code executes
		$('.error').fadeOut('slow'); // reset the error messages (hides them)
		...
		var message = $('textarea#message').val(); // get the value of the input field
		if(message == "" || message == " " || message == "Por favor, entre em contato comigo atraves desse e-mail ou no telefone:") {
			$('#err-message').fadeIn('slow'); // show the error message
			error = true; // change the error state to true
		} 
		...
		var data_string = $('#ajax-formd').serialize(); // Collect data from form
		//alert("here");
		//alert(data_string);

		$.ajax({
			type: "POST",
			url: $('#ajax-formd').attr('action'),
			data: data_string,
			timeout: 6000,
			error: function(request,error) {
				if (error == "timeout") {
        ...
        $("#name").val("Nome");
        $("#email").val("Email");
        $("#message").val("Por favor, entre em contato comigo atraves desse e-mail ou no telefone:");
			}
		});
